...
function renderTop() {
  ...
  const items = [
    ['🪙', 'Treasury', `${fmt(h.figures.treasury.v)} ${trend}`, 'economy', `${h.figures.treasury.src} · ${h.figures.treasury.asOf}${h.figures.debt?.v ? '\nDebt: ' + fmt(h.figures.debt.v) : ''}`],
    ['⚖', 'Per moon', `${pr.low >= 0 ? '+' : ''}${fmt(pr.low)}…${fmt(pr.high)}`, 'economy', 'Steward\'s projection: income minus expenses. Luck, harvests and loyal (or disloyal) vassals decide the real figure.'],
    ['⚔', 'Levies', `~${fmt(h.figures.levies.v)}`, 'military', `Your own unraised levies (${h.figures.levies.src}).\nWith all vassals, if they answer: ~${fmt(tot.levies)}`],
...
async function showSettings() {
  ...
  modal(`<h2>Model settings</h2>
    ...
    <div class="grid2">
      <div><label>Provider</label><select id="cfg-provider"><option value="openai">OpenAI-compatible (local server)</option><option value="mock">Mock (no model, for testing)</option></select></div>
