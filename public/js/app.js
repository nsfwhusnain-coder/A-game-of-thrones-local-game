...
async function showSettings() {
  ...
  const collect = () => {
    ...
    return { provider: $('#cfg-provider').value, model: $('#cfg-model').value.trim(), baseUrl: $('#cfg-url').value.trim(), apiKey: $('#cfg-key').value, contextTokens: Number($('#cfg-ctx').value), maxTokens: Number($('#cfg-max').value), temperature: Number($('#cfg-temp').value), consolidateEvery: Number($('#cfg-cons').value), keepRecentTurns: Number($('#cfg-keep').value), timeoutSec: Number($('#cfg-timeout').value), jsonMode: $('#cfg-json').checked, promptDetail: $('#cfg-detail').value, thinking: $('#cfg-think').value, thinkingBudget: Number($('#cfg-tbudget').value) || 0, stream: $('#cfg-stream').checked, extraBody: extra };
  };
